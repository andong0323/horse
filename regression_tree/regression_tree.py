...
class RegressionTree(object):
	...
	def _get_split_mse(self, train, y, idx, feature, split):
	...
	def _choose_split(self, train, y, idx, feature):
		...
		unique = set([train[i][feature] for i in idx])
		if len(unique) == 1:
			return None
		...
		mse, split, split_avg = min(
			(self._get_split_mse(train, y, idx, feature, split)
				for split in unique), key=lambda x:x[0])
		return mse, feature, split, split_avg

	def _choose_feature(self, train, y, idx):
		...
		split_rets = map(lambda j: self._choose_split(train, y, idx, j), range(m))
	...
	def fit(self, train, y, max_depth=5, min_samples_split=2):
		...
		Keyword Arguments:
			...
			min_samples_split {int} -- The maximum number of samples required
