...
class QuadraticCost(object):
    ...
    @staticmethod 
    def delta(z, a, y):#一阶导数first derivative
        '''
        损失函数对z求偏导
        :param z:x的线性函数
        :param a
...
class FM(object):
    def __init__(self, train, valid, k, eta, maxecho, r2, cost=QuadraticCost):
        ...
        self.valid_x = train[:, :-1]
        self.valid_y = train[:, -1:]
    ...
    def shuffle_data(self):
        ...
        self.train_y = self.train_yp[ids]
    ...
    def predict(self, x):
        ...
        z = self.w0 + np.dot(self.w, x.T).T + np.longlong(
            np.sum((np.dot(x, self.v) ** 2 - np.dot(x ** 2, self.v ** 2)), axis=1).reshape(len(x), 1)) / 2.0
        return z, sigmoid(z)
    ...
    def update_mini_batch(self, x, y, eta):
        ...
        for i in range(batch):
            xi = x[i:i+1, :] #mini batch中的第i个样本。为保持shape不变,注意这里不能用x[i]
...
def fake_data(sample, dim, k):
    ...
    z = w0 + np.dot(w, x.T).T + np.longlong(
        np.sum((np.dot(w, v) ** 2 - np.dot(x ** 2, v ** 2)), aixs=1).reshape(len(x), 1)) / 2.0
    y = sigmoid(z)
