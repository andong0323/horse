...
if __name__ == "__main__":
    ...
    my_ftrl = ftrl(dim=d,l1=1.0,l2=1.0,alpha=0.1,beta=1.0)
    my_ftrl.train(corpus, verbos=False, max_itr=100000, eta=0.01, epochs=100)
    w = my_ftrl.w
    print(w)
    ...
    wrong = 0
    for x, y in corpus:
        y_hat = 1.0 if my_ftrl.predict(x) > 0.5 else 0.0
        if y == y_hat:
            correct += 1
        else:
            wrong += 1
